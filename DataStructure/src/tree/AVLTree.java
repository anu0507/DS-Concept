...
public class AVLTree {
	...
	/**
	 * 1. make a new node e.g; Node newRoot = currentNode.left; 2. separate the
	 * current node e.g; currentNode.left=currentNode.left.right; 3. add current
	 * node to the right of new node 4. update current node's height 5. update new
	 * node's height
	 * 
	 * @param currentNode
	 * @return new node
	...
	private int checkBalance(Node rootLeft, Node rootRight) {
		// if current node is a leaf node then no need to check balance of it's
		// children.
		if (rootLeft == null && rootRight == null) {
			return 0;
		}
		/*
		 * if right node is not present then simply return (left node's height+1) (we
		 * need to make it -1 because blank height is considered as -1) and if left node
		 * is not present then return -(right node's height+1).
		 */
